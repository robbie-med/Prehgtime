...
var ERAS = [
{ id: “era-t1”,   label: “1st Tri”,          start: 1,  end: 13,  cls: “t1”   },
{ id: “era-t2”,   label: “2nd Tri”,          start: 14, end: 27,  cls: “t2”   },
{ id: “era-t3”,   label: “3rd Tri”,          start: 28, end: 36,  cls: “t3”   },
{ id: “era-term”, label: “Term”,             start: 37, end: 42,  cls: “term” }
];

var LANE_ORDER = [“labs”, “appt”, “infect”, “fetal”, “tx”, “screen”];

var TYPE_LABELS = {
labs:    “Labs”,
appt:    “Appointments”,
infect:  “Infections / TORCH”,
fetal:   “Fetal Development”,
tx:      “Treatments / Vaccines”,
screen:  “Screening / Imaging”
};

var TYPE_LABELS_FULL = {
labs:    “Laboratory”,
appt:    “Appointment”,
...
item.innerHTML = ‘<span class="leg-dot" style="background:' + TYPE_COLORS[type] + '"></span>’ + TYPE_LABELS[type];
item.addEventListener(“click”, function () { toggleFilter(type); });
...
activeFilter = null;
document.querySelectorAll(”.lane”).forEach(function (l) { l.classList.remove(“filtered-out”); });
document.querySelectorAll(”.leg-item”).forEach(function (l) { l.classList.remove(“dimmed”); });
} else {
activeFilter = type;
document.querySelectorAll(”.lane”).forEach(function (l) {
l.classList.toggle(“filtered-out”, l.dataset.type !== type);
});
document.querySelectorAll(”.leg-item”).forEach(function (l) {
...
/* — WEEK AXIS — */
function buildAxis() {
var axis = document.getElementById(“week-axis”);
axis.style.height = totalH + “px”;
for (var wk = W0; wk <= W1; wk++) {
var tick = el(“div”, “wk-tick”);
tick.style.top = wY(wk - W0) + “px”;
tick.textContent = wk;
var era = ERAS.find(function (e) { return e.start === wk; });
if (era) {
tick.classList.add(“era-start”);
var tag = el(“span”, “era-tag”, era.label);
...
var container = document.getElementById(“era-bands”);
...
var lbl = el(“div”, “era-label-overlay”);
lbl.textContent = era.label;
lbl.id = era.id;
lbl.style.top = wY(era.start - W0) + 8 + “px”;
...
var hdr = document.getElementById(“lane-headers”);
LANE_ORDER.forEach(function (type) {
var h = el(“div”, “lane-hdr”);
h.style.background = TYPE_COLORS[type];
...
var byType = {};
LANE_ORDER.forEach(function (t) { byType[t] = []; });
...
// Week gridlines
for (var wk = W0; wk <= W1; wk++) {
  var gl = el("div", "week-gridline");
  gl.style.top = wY(wk - W0) + "px";
  lane.appendChild(gl);
}

// Events with collision avoidance
var occupied = [];
var EVT_MIN_H = 80;
var GAP = 4;
var evts = byType[type].slice().sort(function (a, b) { return a.week - b.week; });
...
evts.forEach(function (ev) {
  ...
  var tries = 0;
  while (tries < 60) {
    var bottom = top + EVT_MIN_H;
    var hit = occupied.some(function (p) { return top < p[1] + GAP && bottom + GAP > p[0]; });
    if (!hit) break;
    top += 8;
    tries++;
  }
  occupied.push([top, top + EVT_MIN_H]);

  var wkLabel = Number.isInteger(ev.week) ? "Wk " + ev.week : "Wk " + Math.floor(ev.week) + "\u2013" + (Math.floor(ev.week) + 1);

  var block = el("div", "evt");
  block.style.top = top + "px";
  block.innerHTML =
    '<div class="evt-wk">' + wkLabel + '</div>' +
    '<div class="evt-title">' + ev.title + '</div>' +
    '<div class="evt-preview">' + ev.preview + '</div>';
...
card.innerHTML =
‘<div class="evt-tag" style="color:' + TYPE_COLORS[ev.type] + '">’ + (TYPE_LABELS_FULL[ev.type] || ev.type) + ‘</div>’ +
‘<div class="evt-title">’ + ev.title + ‘</div>’ +
‘<div class="evt-preview">’ + ev.preview + ‘</div>’;
...
document.getElementById(“modal-tag”).textContent = TYPE_LABELS_FULL[ev.type] || ev.type;
